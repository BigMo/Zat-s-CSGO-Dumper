...
namespace Zat_s_CSGO_Dumper
{
    public class SigScanner
    {
        ...
        public SigScanner(Process proc, IntPtr addr, int size)
        {
        ...
        private bool MaskCheck(int nOffset, byte[] btPattern, string strMask, bool wNonZero = false)
        {
            ...
            for (int x = 0; x < btPattern.Length; x++)
            {
                ...
                if (strMask[x] == '?')
                    if (wNonZero && this.m_vDumpedRegion[nOffset + x] == 0x00)
                        return false;
                    else
                        continue;
        ...
        public IntPtr FindPattern(byte[] btPattern, string strMask, int nOffset, bool wNonZero = false)
        {
            try
            {
                ...
                {
                    if (this.MaskCheck(x, btPattern, strMask, wNonZero))
                    {
